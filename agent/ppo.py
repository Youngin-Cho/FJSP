...
class RollOutMemory:
    ...
    def get(self, flags):
        ...
        return fea_gs, fea_pairs, mask_pairs, actions, rewards, values, dones, log_probs
...
class Agent:
    ...
    def update(self, last_state, flags):
        fea_gs, fea_pairs, mask_pairs, actions, rewards, values, dones, log_probs = self.memory.get(flags)
        ...
        for _ in range(self.K_epoch):
            ...
            loss = (- self.P_coeff * torch.min(surr1, surr2)
                    + self.V_coeff * smooth_l1_loss(new_values[flags.flatten()].squeeze(-1), td_target[flags].flatten())
                    - self.E_coeff * entropys)
